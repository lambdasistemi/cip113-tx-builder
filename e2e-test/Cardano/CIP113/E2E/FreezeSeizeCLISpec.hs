...
module Cardano.CIP113.E2E.FreezeSeizeCLISpec (spec) where
...
spec :: Spec
spec =
    describe "CIP-113 freeze/seize CLI subprocess smoke" $ do
        it "freezes a smart-wallet token, signs, submits, and accepts it" $
            runSmoke "freeze"
        it "seizes a smart-wallet token, signs, submits, and accepts it" $
            runSmoke "seize"
...
runSmoke operation = do
    ...
    withCIP113DevnetSocket $ \socketPath lsq ltxs -> do
        ...
        withSystemTempDirectory ("cip113-cli-" <> operation <> "-smoke") $ \tmpDir -> do
            let deploymentFile = tmpDir </> "deployment.json"
                ...
                destinationWallet =
                    smartWalletAddr
                        ...
                        dPlbHash
                        genesisStakeCredential
                targetWalletHex = hex (serialiseAddr targetWallet)
                destinationWalletHex = hex (serialiseAddr destinationWallet)
            BSL.writeFile deploymentFile (Aeson.encode deployment)
            BSL.writeFile signingKeyFile genesisSigningKeyTextEnvelope

            registerThirdPartyAccount provider submitter pp
            threadDelay 5_000_000
            registeredNodeIn <- registerThirdPartyToken provider submitter pp deployment
            threadDelay 5_000_000
            fundSmartWallet provider submitter pp deployment registeredNodeIn tokenName targetWallet
            threadDelay 5_000_000
            ...
            unsignedHex <-
                runCli
                    ...
                    ( commandArgs
                        ...
                        changeAddressHex
                        targetWalletHex
                        destinationWalletHex
                        tokenName
                        policyIdHex
                    )
                    ""
            signAndSubmit cip113Cli signingKeyFile submitter operation unsignedHex
...
commandArgs ::
    ...
    String ->
    [String]
commandArgs socketPath operation deploymentFile changeAddress targetAddress destinationAddress tokenName policyId =
    [ "--socket-path"
    , socketPath
    , "--network-magic"
    , "42"
    ...
    , policyId
        ...
        <> if operation == "seize"
            then ["--to-address", destinationAddress]
            else []
...
registerThirdPartyAccount ::
    ...
    PParams ConwayEra ->
    IO ()
registerThirdPartyAccount provider submitter pp = do
...
registerThirdPartyToken ::
    Provider IO ->
    Submitter IO ->
    PParams ConwayEra ->
    CIP113Deployment ->
    IO TxIn
registerThirdPartyToken provider submitter pp CIP113Deployment{..} = do
    ...
    let testPolicyKey = case dIssuancePolicy of
        ...
        thirdPartyCred = case genesisStakeCredential of
